...
To retrieve the current time parameters from the device, use the request ``{"cmd": 3}`` `Command <commands.html>`_.
...
``time`` - time in seconds since 01.01.2000 00:00 in UTC
